$(document).ready(function(){
    ...
    var $form = $('#form');
    var $procContainer = $('#proc_container');
    ...
    //新增或编辑节点的弹窗的html字符串
    var procItemModalHtml = '<div class="row-space-10">' +
    ...
    * @param String mode 编辑模式='edit',查看模式='view'
    **/
    function procBuilder(data, $container, mode) {
        var procHtmlString =    '<div class="proc-content">' +
        ...
        $container.append(procHtmlString);
        
        procData = data ? data : [];
        if(procData.length == 0) {
            if(mode == 'edit') {
                addProcNewBtn(data, $container);
            } else {
                emptyHtml = '<div class="proc-item-container">' +
                                '<div class="proc-item">流程数据为空！</div>' +
                            '</div>';
                $container.find('.proc-item-start').eq(0).after(emptyHtml);
            }
        } else {
            var itemsHtml = '';
            for(var i = 0; i < data.length; i++) {
                (function(i) {
                    itemsHtml += createOneProcItemString(data[i], mode, i);
                })(i);
            }
            $container.find('.proc-item-start').eq(0).after(itemsHtml);
            if(mode == 'edit') {
                var procItemNodes = $container.find('.proc-item-node');
                procItemNodes.each(function(){
                    addBtnActions(procData, $container, $(this));
                });
            }
        }
    }
    ...
    **/
    ...
    /**
    * 刷新流程节点序号，并根据节点数量调整显示内容（节点数量为空，显示添加节点按钮，节点数量不为空删除这个按钮）
    * @param Array procData 流程数据
    * @param Object $container jquery选择器 流程图的dom容器
    **/
    function refreshProcIndex(procData, $container) {
        var procNodes = $container.find('.proc-item-node');
        var procNewBtns = $container.find('.proc-item-btn');
        //判断节点数量是否为空，如果为空且没有“点击添加新节点”的按钮，添加这个按钮，
        //如果不为空且这个按钮存在，则删除这个按钮;刷新节点index
        if(procNodes.length == 0) {
            if(procNewBtns.length == 0) {
                addProcNewBtn(procData, $container);
            }
        } else {
    ...
    * @param Object $container jquery选择器 流程图的dom容器
    **/
    function addProcNewBtn(procData, $container) {
        var newBtnHtml =    '<div class="proc-item-container proc-item-btn">' +
                                '<div class="proc-item">点击添加流程节点</div>' +
                            '</div>';
        $container.find('.proc-item-start').eq(0).after(newBtnHtml);
        //添加按钮交互
        $container.find('.proc-item-btn .proc-item').click(function(){
            addOneProcItem(procData, $container, $container.find('.proc-item-start').eq(0), 'after');
        });
    }
    
    /**
    * 弹出添加节点窗口，添加流程节点
    * @param Array procData 当前流程图的数据
    * @param Object $container jquery选择器 流程图的dom容器
    * @param Object $referenceItem 参考节点的jquery选择器对象，将基于此参考节点的前或后添加新节点
    * @param String addPosition 前添加='before'，后添加='after'
    **/
    function addOneProcItem(procData, $container, $referenceItem, addPosition) {
        BSModal.confirm({ 
            title : "添加节点", 
            content : procItemModalHtml, 
            width : "460px", 
            btnOKDismiss : false,
            afterInit : function($modal){ //弹窗初始化完成后，添加相关表单控制
                setExaminerSelect($modal, examiners);
            }
        }).on(function (e, id) {
            if (!e) {
                return;
            ...
            if($modal.find('.proc-item-name').eq(0).val().length == 0) {
                ...
                if($referenceItem.attr('data-index')){ //如果有data-index属性，说明原流程图节点数量不为0
                    ...
                    if(addPosition == 'after') {
                        ...
                        targetIndex += 1;
                        $referenceItem.after(createOneProcItemString(procItemData, 'edit'));
                    } else {
                        $referenceItem.before(createOneProcItemString(procItemData, 'edit'));
                    }
                    procData.splice(targetIndex, 0, procItemData); //插入新节点数据
                    ...
                    procData.push(procItemData);
                }
                refreshProcIndex(procData, $container); //刷新dom里的节点index,与添加后的节点数据同步
                addBtnActions(procData, $container, $container.find('.proc-item-node').eq(targetIndex)); //添加节点的按钮交互
                $modal.modal('hide');
            }
        });
    }
    ...
    **/
    function removeOneProcItem(procData, $container, $procItemNode) {
        BSModal.confirm({ 
            title : "删除节点", 
            content : '确认要删除该流程节点吗？', 
            width : "320px",
            ...
            $procItemNode.remove();
            refreshProcIndex(procData, $container);
            $('#' + id).modal('hide');
        });
    }
    ...
    * @param Object $procItemNode 节点的jquery选择器对象，将基于此参考节点的前或后添加新节点
    **/
    function editOneProcItem(procData, $container, $procItemNode) {
        var procItemNodeData = procData[$procItemNode.attr('data-index')];
        BSModal.confirm({ 
            title : "编辑节点", 
            content : procItemModalHtml, 
            width : "460px",
            ...
            afterInit : function($modal) { //弹窗初始化完成后，添加相关表单控制
                ...
                setExaminerSelect($modal, examiners, selectedData);
            ...
            if($modal.find('.proc-item-name').eq(0).val().length == 0) {
                ...
                if(_.isEqual(procItemData, procItemNodeData)) {
                    ...
                    procData.splice($procItemNode.attr('data-index'), 1, procItemData); //替换节点数据
                    $modal.modal('hide');
                }
            }
        });
    ...
    * @param Object $procItemNode 节点的jquery选择器对象，将基于此参考节点的前或后添加新节点
    **/
    function addBtnActions(procData, $container, $procItemNode) {
        //alert('addBtnActions');
        ...
        btnAddBefore.click(function(){
            //alert('btnAddBefore');
            addOneProcItem(procData, $container, $procItemNode, 'before');
        });
        btnAddAfter.click(function(){
            //alert('btnAddAfter');
            addOneProcItem(procData, $container, $procItemNode, 'after');
        });
        btnRemove.click(function(){
            removeOneProcItem(procData, $container, $procItemNode);
        });
        btnEdit.click(function(){
            editOneProcItem(procData, $container, $procItemNode);
        });
    }
    ...
    * @param Object selectedData 需要填入的选定的数据，编辑弹窗使用
    **/
    function setExaminerSelect($modal, examinerData, selectedData) {
        var depList = [{value : '不选择', text : '不选择'}];
        var $selectDep = $modal.find('.proc-item-dep').eq(0);
        var $selectExaminer = $modal.find('.proc-item-examiner').eq(0);
    ...
    procBuilder(proc_data, $procContainer, 'edit');
