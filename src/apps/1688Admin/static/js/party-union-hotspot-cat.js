$(document).ready(function(){
    ...
    function setBSTable() {
        ...
        $btn_new.click(function() {
            BSModal.confirm({ title:"新增热点聚焦类型", content:getStateNameHtml(), width:"60%", btnOKDismiss:false}).on(function (e, id) {
                if (!e) {
                    return;
                }
                ...
                if($('#'+id).find('#cat_name').eq(0).val().length == 0) {
                    toastr.warning('请输入热点聚焦类型名称');
                } else {
                    $('#'+id).modal('hide');
                    toastr.success('添加热点聚焦类型成功');
                    //此处提交热点聚焦类型名称，并刷新表格...
                }
        ...
        $btn_edit.click(function () {
            if(!$(this).attr('disabled')) {
                ...
                BSModal.confirm({ title:"编辑热点聚焦类型", content:getStateNameHtml(currentCatName), width:"60%", btnOKDismiss:false}).on(function (e, id) {
                    if (!e) {
                        return;
                    }
                    ...
                    if($('#'+id).find('#cat_name').eq(0).val().length == 0) {
                        ...
                        toastr.warning('热点聚焦类型名称未修改');
                    } else {
                        $('#'+id).modal('hide');
                        toastr.success('修改热点聚焦类型成功');
                        //此处提交热点聚焦类型名称，并刷新表格...
                    }
